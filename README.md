...
The project has been developed to run from WSL. The Makefile currently uses the WSL path:

```text
/mnt/m/github_project/Python/Enabling_AI-Powered_Business_Intelligence_for_Organizations
```

If the project is moved, update the `PYTHONPATH` path in `Makefile` or run commands with your own `PYTHONPATH`.
...
```bash
cd /mnt/m/github_project/Python/Enabling_AI-Powered_Business_Intelligence_for_Organizations
make setup
```
